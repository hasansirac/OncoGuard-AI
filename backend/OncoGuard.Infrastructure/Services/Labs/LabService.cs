...
public class LabService : ILabService
{
    ...
    public async Task AddLabResultAsync(CreateLabResultRequest request)
    {
        ...
        var labDate = request.LabDate.Date;
        ...
        if (activeCycle != null)
        {
            ...
            activeCycle.EndDate = labDate > activeCycle.StartDate.Date
                ? labDate.AddDays(-1)
                : labDate;
        ...
        var newCycle = new LabCycle
        {
            ...
            StartDate = labDate,
            EndDate = labDate.AddDays(cycleLengthDays - 1),
            Status = CycleStatus.Active,
            PreviousCycleId = activeCycle?.Id,
            CycleSummary = $"New {cycleLengthDays}-day monitoring cycle started with this lab result as baseline."
        };
